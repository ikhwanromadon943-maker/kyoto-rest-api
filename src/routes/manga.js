...
// Search / Chapter / Anime / Random : api.jikan.moe/v4
  ...
  try {
    ...
    if (endpoint === 'search') {
      ...
      try {
        ...
        if (!ext.ok) {
          return res.status(ext.status).json({
            status: false, author: 'Kyoto API',
            error: `Jikan API returned ${ext.status} — mungkin rate limited, coba lagi nanti.`,
            timestamp: new Date().toISOString(), response_time: `${Date.now() - start}ms`
          });
        }
        ...
        return res.status(500).json({
          status: false, author: 'Kyoto API',
          error: `Jikan API request failed: ${err.message}`,
          timestamp: new Date().toISOString(), response_time: `${Date.now() - start}ms`
        });
      }
    }
    ...
    if (endpoint === 'chapter') {
      ...
      try {
        ...
        if (!ext.ok) {
          return res.status(ext.status).json({
            status: false, author: 'Kyoto API',
            error: `Jikan API returned ${ext.status} — manga not found or rate limited.`,
            timestamp: new Date().toISOString(), response_time: `${Date.now() - start}ms`
          });
        }
        const data = await ext.json();
    ...
    if (endpoint === 'anime') {
      ...
      try {
        ...
        if (!ext.ok) {
          return res.status(ext.status).json({
            status: false, author: 'Kyoto API',
            error: `Jikan API returned ${ext.status} — mungkin rate limited.`,
            timestamp: new Date().toISOString(), response_time: `${Date.now() - start}ms`
          });
        }
        const data = await ext.json();
    ...
    if (endpoint === 'random') {
      ...
      return res.status(500).json({
        status: false, author: 'Kyoto API',
        error: 'Failed to fetch random manga after multiple attempts',
        timestamp: new Date().toISOString(), response_time: `${Date.now() - start}ms`
      });
    }
    ...
    if (endpoint === 'waifu') {
      ...
      try {
        ...
        return res.status(500).json({
          status: false, author: 'Kyoto API',
          error: `Waifu.pics request failed: ${err.message}`,
          timestamp: new Date().toISOString(), response_time: `${Date.now() - start}ms`
        });
      }
    }
