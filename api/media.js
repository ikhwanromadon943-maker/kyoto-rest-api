  ...
  try {
    ...
    if (ep === 'wallpaper') {
      ...
      return res.json({
        ...
        theme, width, height, seed,
        url: `https://image.pollinations.ai/prompt/${encodeURIComponent(theme)}?width=${width}&height=${height}&seed=${seed}&nologo=true`,
        response_time: rt()
      });
    }
    ...
    if (ep === 'art') {
      ...
      return res.json({
        status: true, author: 'Kyoto API', provider: 'Pollinations.ai',
        subject, style, seed,
        url: `https://image.pollinations.ai/prompt/${encodeURIComponent(subject + ' in ' + style + ' style')}?width=512&height=512&seed=${seed}&nologo=true`,
        response_time: rt()
      });
    }
    ...
    if (ep === 'banner') {
      ...
      return res.json({
        status: true, author: 'Kyoto API', provider: 'Pollinations.ai',
        text,
        url: `https://image.pollinations.ai/prompt/${encodeURIComponent(style + ' text: ' + text)}?width=1200&height=400&nologo=true`,
        response_time: rt()
      });
    }
    ...
    if (ep === 'thumbnail') {
      ...
      return res.json({
        status: true, author: 'Kyoto API', provider: 'Pollinations.ai',
        title,
        url: `https://image.pollinations.ai/prompt/${encodeURIComponent(bg + ' youtube thumbnail text: ' + title)}?width=1280&height=720&nologo=true`,
        response_time: rt()
      });
    }

    // NEW: YouTube video info via Noembed
    if (ep === 'youtube') {
      ...
      try {
        ...
        return errRes(res, 'Failed to fetch YouTube info. Check URL format.');
      }
    }

    // NEW: Unsplash random photo
    if (ep === 'unsplash') {
      const query = url.searchParams.get('query') || 'nature';
      try {
        // Using Unsplash source API (free, no key)
        ...
        return res.json({
          ...
          query,
          url: imageUrl,
          note: 'Image URL redirects to random high-quality photo',
          response_time: rt()
        });
      } catch {
        return errRes(res, 'Failed to fetch Unsplash image.');
      }
    }
    ...
    if (ep === 'canvas') {
      ...
      return res.json({
        status: true, author: 'Kyoto API', provider: 'Pollinations.ai',
        image: `https://image.pollinations.ai/prompt/${encodeURIComponent(t)}?width=800&height=400&nologo=true`,
        response_time: rt()
      });
    }
    ...
    if (ep === 'quote-image') {
      ...
      return res.json({
        status: true, author: 'Kyoto API', provider: 'Pollinations.ai',
        image: `https://image.pollinations.ai/prompt/${encodeURIComponent(q)}%20-%20${encodeURIComponent(a)}?width=800&height=400&nologo=true`,
        response_time: rt()
      });
    }

    return errRes(res, `Endpoint /api/media/${ep} not found. Available: wallpaper, art, profile-pic, banner, thumbnail, youtube, unsplash, canvas, meme, quote-image`);
